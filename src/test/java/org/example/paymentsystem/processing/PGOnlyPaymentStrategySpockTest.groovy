...
class PGOnlyPaymentStrategySpockTest extends Specification {
    ...
    PGOnlyPaymentStrategy strategy

    def setup() {
        strategy = new PGOnlyPaymentStrategy(paymentGatewayService, orderStatusService, orderService, transactionService)
    }

    def "PG결제(amount)가 전액일 경우에만 지원한다"() {
        expect:
